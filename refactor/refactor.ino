...
const int HOMING_DISTANCE =
    60; // Distância para começar a perseguição agressiva
const int LOST_TRACK_THRESHOLD = 3; // Medições consecutivas sem detectar o alvo
const int MAX_SEARCH_SWEEPS =
    3; // Número máximo de varreduras para recuperar o alvo
...
void setup()
{
	...
	attachInterrupt(digitalPinToInterrupt(linhaDir), handleLineDetection,
			RISING);
	attachInterrupt(digitalPinToInterrupt(linhaEsq), handleLineDetection,
			RISING);
